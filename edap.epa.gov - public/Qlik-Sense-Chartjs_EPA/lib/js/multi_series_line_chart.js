var visualize = function($element, layout, _this, chartjsUtils) {
  ...
  for(var i=0; i<dim2_unique_values.length; i++ ) {
    ...
    if (!qDimColorUser[1].auto) {
      ...
      subdata.borderColor = custompalette[dim2_unique_values[i]];
    } else {
          ...
          onClick: function(evt) {
              ...
              if(activePoints.length > 0) {
                if(layout.selectdim2) {
                  console.log(Object.keys(dim2_unique_values).map(function(e) { return dim2_unique_values[e] })[activePoints[0]._datasetIndex]);
                  chartjsUtils.makeSelectionsOnDataPoints(dim2_unique_elem_nums[Object.keys(dim2_unique_values).map(function(e) { return dim2_unique_values[e] })[activePoints[0]._datasetIndex]], _this);
                } else {
