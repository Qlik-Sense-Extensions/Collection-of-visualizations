...
/* 		snapshot: {
		...
		paint: function ( $element , layout) {
			...
			var n = vars.id, styles = vars.styletype; styles = styles.toLowerCase(), tempId[n] = [];
			...
			// Button	
				...
				html += '<div class="btn-group" '+spn+'>'
					this.backendApi.eachDataRow(function(rownum, row) {	
						if(row[0].qState==='S'){var active ='btn-'+btnColor; tempId[n].push(row[0].qElemNumber);}else{var active ='btn-default';};
						if(row[0].qState==='X'){var dis = 'disabled';}else{var dis = '';};
			...
			$('#'+vars.id+' .form-control').change(function(d) {				
				...
				if(vars.dropMultiSelect){										
					...
					if(vars.oneSelected){tempId[n] = [lastChar]; vars.bool = false}else{vars.bool = true};	
					tempId[n] = _.uniq(tempId[n]);				
