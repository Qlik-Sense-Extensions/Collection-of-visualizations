if (!window.jQuery) {
	$("#msg"+layout.qInfo.qId).text("Could not find jQuery! The HTML must include jquery.js before opencpu.js!");
...
(function($) {
    ...
    function Session(loc, key, txt) {
        ...
        this.getObject = function(name, data, success) {
            ...
            var url = this.getLoc() + "R/" + name + "/json?force=true";
    ...
    function testhtml5() {
        if (window.FormData === undefined) {
			$("#msg"+layout.qInfo.qId).text("Uploading of files requires HTML5. It looks like you are using an outdated browser that does not support this. Please install Firefox, Chrome or Internet Explorer 10+");
    ...
    function seturl(newpath) {
        if (!newpath.match("/R$")) {
			$("#msg"+layout.qInfo.qId).text("ERROR! Trying to set R url to: " + newpath + ". Path to an OpenCPU R package must end with '/R'");
            ...
            if (location.protocol != r_path.protocol || location.host != r_path.host) {
                ...
                if (!('withCredentials' in new XMLHttpRequest())) {
					$("#msg"+layout.qInfo.qId).text("This browser does not support CORS. Try using Firefox or Chrome.");
                    //alert("This browser does not support CORS. Try using Firefox or Chrome.");
                }
            }
            if (location.protocol == "https:" && r_path.protocol != "https:") {
				$("#msg"+layout.qInfo.qId).text("Page is hosted on HTTPS but using a (non-SSL) HTTP OpenCPU server. This is insecure and most browsers will not allow this.");
            ...
            $.get(r_path.href, function(resdata) {
				...
				$("#msg"+layout.qInfo.qId).text("Connection to OpenCPU failed:\n" + textStatus + "\n" + xhr.responseText + "\n" + errorThrown);
